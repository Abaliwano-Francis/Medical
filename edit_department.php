...
if (isset($_GET['id'])) {
    $idFromUrl = $_GET['id'];
    $sql = mysqli_query($con, "SELECT * FROM `departments` WHERE `dept_id`='$idFromUrl'");
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                <div class="form-group">
                                    <input name="name" type="text" class="form-control" value="<?php echo $resultsr['name'] ?>">
                                </div>
                            </div>
                           </div>
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                <div class="row">
                                    ...
                                    <a class="btn btn-danger waves-effect waves-light" href="departments.php">Cancel</a>
                                </div>
                                <div class="payment-adress col-lg-6 col-md-6 col-sm-6 col-xs-6">
...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_query($con, "UPDATE `departments` SET `name`='$name' WHERE `dept_id`='$idFromUrl'");
    if ($query) {
        echo "<script>alert('Department Updated successfully')
        window.location='departments.php'</script>";
    }
    else{
        echo "<script>alert('Failed to Update Department')
        window.location='departments.php'</script>";
    }
}
